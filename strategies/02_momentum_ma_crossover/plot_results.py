...
def plot_momentum(df, short_ma, long_ma):
    setup_style()
    ...
    ax_price.plot(df.index, df["price"], color=COLORS["price"], lw=1.0, label="GARAN.IS Close", alpha=0.6)
    ax_price.plot(df.index, short_ma, color=COLORS["strategy"], lw=1.2, label="20-day MA")
    ax_price.plot(df.index, long_ma, color=COLORS["buy_and_hold"], lw=1.2, label="50-day MA")
    shade_positions(ax_price, df.index, df["position"])

    ax_price.set_title("Momentum (MA Crossover) — Price and Positions", fontsize=12, fontweight="bold")
...
if __name__ == "__main__":
    ...
    plot_momentum(result, short_ma, long_ma)
